...
def create_app(
    ...
    async def healthz() -> dict[str, Any]:
        ...
        """
        return {"status": "ok", "models": config.model_names}
    ...
    async def list_models() -> ModelList:
        return ModelList(data=[ModelCard(id=name) for name in config.model_names])

    @app.post("/v1/chat/completions")
    async def chat_completions(body: ChatCompletionRequest, http: Request) -> Any:
        engine = _engine(app)
        if config.model_by_name(body.model) is None:
            return JSONResponse(
                status_code=404,
                content=error_body(
                    f"model {body.model!r} is not configured; "
                    f"available: {', '.join(config.model_names)}",
        ...
        queued = engine.submit(body.to_generation_request(request_id=""))
...
async def _collect(
    ...
    return ChatCompletionResponse(
        ...
        usage=Usage(
            prompt_tokens=len(queued.request.prompt.split()),
            completion_tokens=len(parts),
            total_tokens=len(queued.request.prompt.split()) + len(parts),
        ),
    )
